 ...
 * Cart
 *
 * The WooCommerceKlaviyo cart class handles saved cart data. It's an extension to WooCommerce.
 *
 * @class     WCK_Cart
 * @version   2.1.0
...
class WCK_Cart {
  ...
  /** @public int Cart (post) ID */
  public $id;

  /**
   * Get the cart if ID is passed, otherwise the order is new and empty.
   *
   * @access public
  ...
  /**
   * Populates a cart from the loaded post data.
   *
   * @access public
   * @param mixed $result
   * @return void
  ...
  public function populate( $result ) {
    ...
    $this->id                = $result->ID;
    $this->started_date      = $result->post_date;
    $this->modified_date     = $result->post_modified;

    // Get status
    $terms        = wp_get_object_terms( $this->id, 'klaviyo_shop_cart_status', array( 'fields' => 'slugs' ) );
    $this->status = isset( $terms[0] ) ? $terms[0] : apply_filters( 'woocommerce_default_cart_status', 'active' );
  }
