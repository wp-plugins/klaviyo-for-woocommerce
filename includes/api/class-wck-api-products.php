<?php
/**
 * WooCommerceKlaviyo API Products Class
 *
...
class WCK_API_Products extends WCK_API_Resource {
	...
	public function register_routes( $routes ) {
		...
		$routes[ $this->base ] = array(
			array( array( $this, 'get_products' ),     WCK_API_Server::READABLE ),
		);

		# GET /products/count
		$routes[ $this->base . '/count'] = array(
			array( array( $this, 'get_products_count' ), WCK_API_Server::READABLE ),
		);

		# GET /products/<id>
		$routes[ $this->base . '/(?P<id>\d+)' ] = array(
			array( array( $this, 'get_product' ),  WCK_API_Server::READABLE ),
		);

		# GET /products/<id>/reviews
		$routes[ $this->base . '/(?P<id>\d+)/reviews' ] = array(
			array( array( $this, 'get_product_reviews' ), WCK_API_Server::READABLE ),
		);
	...
	public function get_product( $id, $fields = null ) {
		...
		return array( 'product' => apply_filters( 'woocommerce_klaviyo_api_product_response', $product_data, $product, $fields, $this->server ) );
	}
	...
	public function get_products_count( $type = null, $filter = array() ) {
		...
		if ( ! current_user_can( 'read_private_products' ) )
			return new WP_Error( 'woocommerce_klaviyo_api_user_cannot_read_products_count', __( 'You do not have permission to read the products count', 'woocommerce-klaviyo' ), array( 'status' => 401 ) );
	...
	public function get_product_reviews( $id, $fields = null ) {
		...
		return array( 'product_reviews' => apply_filters( 'woocommerce_klaviyo_api_product_reviews_response', $reviews, $id, $fields, $comments, $this->server ) );
	}
	...
	private function get_images( $product ) {
		...
		if ( empty( $images ) ) {
			...
			$images[] = array(
				...
				'title'      => __( 'Placeholder', 'woocommerce-klaviyo' ),
				'alt'        => __( 'Placeholder', 'woocommerce-klaviyo' ),
				'position'   => 0,
			);
		}
