<?php
/**
 * WooCommerceKlaviyo API Customers Class
 *
...
class WCK_API_Customers extends WCK_API_Resource {
	 ...
	 *
	 * @since 0.9
	 * @param WCK_API_Server $server
	 * @return WCK_API_Customers
	 */
	public function __construct( WCK_API_Server $server ) {

		parent::__construct( $server );

		// add customer data to order responses
		add_filter( 'woocommerce_klaviyo_api_order_response', array( $this, 'add_customer_data' ), 10, 2 );
	...
	public function register_routes( $routes ) {
		...
		$routes[ $this->base ] = array(
			array( array( $this, 'get_customers' ),     WCK_API_SERVER::READABLE ),
		);

		# GET /customers/count
		$routes[ $this->base . '/count'] = array(
			array( array( $this, 'get_customers_count' ), WCK_API_SERVER::READABLE ),
		);

		# GET /customers/<id>
		$routes[ $this->base . '/(?P<id>\d+)' ] = array(
			array( array( $this, 'get_customer' ),  WCK_API_SERVER::READABLE ),
		);

		# GET /customers/<id>/orders
		$routes[ $this->base . '/(?P<id>\d+)/orders' ] = array(
			array( array( $this, 'get_customer_orders' ), WCK_API_SERVER::READABLE ),
		);
	...
	public function get_customer( $id, $fields = null ) {
		...
		return array( 'customer' => apply_filters( 'woocommerce_klaviyo_api_customer_response', $customer_data, $customer, $fields, $this->server ) );
	}
	...
	public function get_customers_count( $filter = array() ) {
		...
		if ( ! current_user_can( 'list_users' ) )
			return new WP_Error( 'woocommerce_klaviyo_api_user_cannot_read_customers_count', __( 'You do not have permission to read the customers count', 'woocommerce-klaviyo' ), array( 'status' => 401 ) );
	...
	public function create_customer( $data ) {
		...
		if ( ! current_user_can( 'create_users' ) )
			return new WP_Error( 'woocommerce_klaviyo_api_user_cannot_create_customer', __( 'You do not have permission to create this customer', 'woocommerce-klaviyo' ), array( 'status' => 401 ) );
	...
	public function get_customer_orders( $id, $fields = null ) {
		...
		foreach ( $order_ids as $order_id ) {
			$orders[] = current( WC()->api->WCK_API_Orders->get_order( $order_id, $fields ) );
		}

		return array( 'orders' => apply_filters( 'woocommerce_klaviyo_api_customer_orders_response', $orders, $id, $fields, $order_ids, $this->server ) );
	}
	 ...
	 *
	 * @since 0.9
	 * @see WCK_API_Resource::validate_request()
	...
	protected function validate_request( $id, $type, $context ) {
		...
		// validate ID
		if ( empty( $id ) )
			return new WP_Error( 'woocommerce_klaviyo_api_invalid_customer_id', __( 'Invalid customer ID', 'woocommerce-klaviyo' ), array( 'status' => 404 ) );

		// non-existent IDs return a valid WP_User object with the user ID = 0
		$customer = new WP_User( $id );

		if ( 0 === $customer->ID )
			return new WP_Error( 'woocommerce_klaviyo_api_invalid_customer', __( 'Invalid customer', 'woocommerce-klaviyo' ), array( 'status' => 404 ) );
		...
		switch ( $context ) {
			...
			case 'read':
				if ( ! current_user_can( 'list_users' ) )
					return new WP_Error( 'woocommerce_klaviyo_api_user_cannot_read_customer', __( 'You do not have permission to read this customer', 'woocommerce-klaviyo' ), array( 'status' => 401 ) );
				break;

			case 'edit':
				if ( ! current_user_can( 'edit_users' ) )
					return new WP_Error( 'woocommerce_klaviyo_api_user_cannot_edit_customer', __( 'You do not have permission to edit this customer', 'woocommerce-klaviyo' ), array( 'status' => 401 ) );
				break;

			case 'delete':
				if ( ! current_user_can( 'delete_users' ) )
					return new WP_Error( 'woocommerce_klaviyo_api_user_cannot_delete_customer', __( 'You do not have permission to delete this customer', 'woocommerce-klaviyo' ), array( 'status' => 401 ) );
				break;
		}
	 ...
	 *
	 * @since 0.9
	 * @see WCK_API_Resource::is_readable()
