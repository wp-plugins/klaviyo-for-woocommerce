<?php
/**
 * WooCommerceKlaviyo API Authentication Class
 *
 * @author      Klaviyo
 * @category    API
...
class WCK_API_Authentication {

	/**
	 * Setup class
	 *
	 * @since 0.9
	 * @return WCK_API_Authentication
	 */
	public function __construct() {

		// to disable authentication, hook into this filter at a later priority and return a valid WP_User
		add_filter( 'woocommerce_klaviyo_api_check_authentication', array( $this, 'authenticate' ), 0 );
	}
	...
	public function authenticate( $user ) {
		...
		try {
			...
			$user = new WP_Error( 'woocommerce_klaviyo_api_authentication_error', $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	...
	private function perform_ssl_authentication() {
		...
		$params = WCK()->api->server->params['GET'];
		...
		if ( ! empty( $_SERVER['PHP_AUTH_USER'] ) ) {
			...
			throw new Exception( __( 'Consumer Key is missing', 'woocommerce-klaviyo' ), 404 );
		}
		...
		if ( ! empty( $_SERVER['PHP_AUTH_PW'] ) ) {
			...
			throw new Exception( __( 'Consumer Secret is missing', 'woocommerce-klaviyo' ), 404 );
		}

		$user = $this->get_user_by_consumer_key( $consumer_key );

		if ( ! $this->is_consumer_secret_valid( $user, $consumer_secret ) ) {
			throw new Exception( __( 'Consumer Secret is invalid', 'woocommerce-klaviyo' ), 401 );
		}
	...
	private function perform_oauth_authentication() {
		...
		foreach ( $param_names as $param_name ) {
			...
			if ( empty( $params[ $param_name ] ) )
				throw new Exception( sprintf( __( '%s parameter is missing', 'woocommerce-klaviyo' ), $param_name ), 404 );
		}
	...
	private function get_user_by_consumer_key( $consumer_key ) {
		...
		if ( empty( $users[0] ) )
			throw new Exception( __( 'Consumer Key is invalid', 'woocommerce-klaviyo' ), 401 );
	...
	private function check_oauth_signature( $user, $params ) {
		...
		$http_method = strtoupper( WCK()->api->server->method );

		$base_request_uri = rawurlencode( untrailingslashit( get_woocommerce_klaviyo_api_url( '' ) ) . WCK()->api->server->path );
		...
		if ( ! uksort( $params, 'strcmp' ) ) {
			throw new Exception( __( 'Invalid Signature - failed to sort parameters', 'woocommerce-klaviyo' ), 401 );
		}
		...
		if ( $signature !== $consumer_signature ) {
			throw new Exception( __( 'Invalid Signature - provided signature does not match', 'woocommerce-klaviyo' ), 401 );
		}
	}
	...
	private function check_oauth_timestamp_and_nonce( $user, $timestamp, $nonce ) {
		...
		if ( ( $timestamp < time() - $valid_window ) ||  ( $timestamp > time() + $valid_window ) )
			throw new Exception( __( 'Invalid timestamp', 'woocommerce-klaviyo' ) );
		...
		if ( in_array( $nonce, $used_nonces ) )
			throw new Exception( __( 'Invalid nonce - nonce has already been used', 'woocommerce-klaviyo' ), 401 );
	...
	public function check_api_key_permissions( $user ) {
		...
		switch ( WCK()->api->server->method ) {

			case 'HEAD':
			case 'GET':
				if ( 'read' !== $key_permissions && 'read_write' !== $key_permissions ) {
					throw new Exception( __( 'The API key provided does not have read permissions', 'woocommerce-klaviyo' ), 401 );
				}
				break;
			...
			case 'DELETE':
				if ( 'write' !== $key_permissions && 'read_write' !== $key_permissions ) {
					throw new Exception( __( 'The API key provided does not have write permissions', 'woocommerce-klaviyo' ), 401 );
				}
				break;
		}
	}
