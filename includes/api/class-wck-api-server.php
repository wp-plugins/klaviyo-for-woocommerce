...
class WCK_API_Server {
	...
	public function __construct( $path ) {
		...
		$this->handler = new WCK_API_JSON_Handler();
	}
	...
	public function check_authentication() {
		...
		$user = apply_filters( 'woocommerce_klaviyo_api_check_authentication', null, $this );
		...
		elseif ( ! is_wp_error( $user ) )
			$user = new WP_Error( 'woocommerce_klaviyo_api_authentication_error', __( 'Invalid authentication method', 'woocommerce-klaviyo' ), array( 'code' => 500 ) );
	 ...
	 *
	 * @since 0.9
	 * @uses WCK_API_Server::dispatch()
	 */
	public function serve_request() {

		do_action( 'woocommerce_klaviyo_api_server_before_serve', $this );

		$this->header( 'Content-Type', $this->handler->get_content_type(), true );

		// the API is enabled by default
		if ( ! apply_filters( 'woocommerce_klaviyo_api_enabled', true, $this ) || ( 'no' === get_option( 'woocommerce_api_enabled' ) ) ) {
		...
		$served = apply_filters( 'woocommerce_klaviyo_api_serve_request', false, $result, $this );
	...
	public function get_routes() {
		...
		$endpoints = apply_filters( 'woocommerce_klaviyo_api_endpoints', $endpoints );
	...
	public function dispatch() {
		...
		switch ( $this->method ) {
			...
			default:
				return new WP_Error( 'woocommerce_klaviyo_api_unsupported_method', __( 'Unsupported request method', 'woocommerce-klaviyo' ), array( 'status' => 400 ) );
		}
		...
		foreach ( $this->get_routes() as $route => $handlers ) {
			foreach ( $handlers as $handler ) {
				...
				if ( ! is_callable( $callback ) )
					return new WP_Error( 'woocommerce_klaviyo_api_invalid_handler', __( 'The handler for the route is invalid', 'woocommerce-klaviyo' ), array( 'status' => 500 ) );
				...
				$args = apply_filters( 'woocommerce_klaviyo_api_dispatch_args', $args, $callback );
		...
		return new WP_Error( 'woocommerce_klaviyo_api_no_route', __( 'No route was found matching the URL and request method', 'woocommerce-klaviyo' ), array( 'status' => 404 ) );
	}
	...
	protected function sort_callback_params( $callback, $provided ) {
		...
		foreach ( $wanted as $param ) {
			...
			elseif ( $param->isDefaultValueAvailable() ) {
				...
				return new WP_Error( 'woocommerce_klaviyo_api_missing_callback_param', sprintf( __( 'Missing parameter %s', 'woocommerce-klaviyo' ), $param->getName() ), array( 'status' => 400 ) );
			}
		}
			...
			foreach ( self::$method_map as $name => $bitmask ) {
				foreach ( $callbacks as $callback ) {
					...
					if ( strpos( $route, '<' ) === false ) {
						$data['meta'] = array(
							'self' => get_woocommerce_klaviyo_api_url( $route ),
						);
					}
				}
			}
			$available['store']['routes'][ $route ] = apply_filters( 'woocommerce_klaviyo_api_endpoints_description', $data );
		}
		return apply_filters( 'woocommerce_klaviyo_api_index', $available );
	}
	...
	public function add_pagination_headers( $query ) {
		...
		$this->header( 'X-WCK-Total', $total );
		$this->header( 'X-WCK-TotalPages', $total_pages );

		do_action( 'woocommerce_klaviyo_api_pagination_headers', $this, $query );
	}
