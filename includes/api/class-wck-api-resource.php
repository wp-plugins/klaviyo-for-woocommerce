<?php
/**
 * WooCommerceKlaviyo API Resource class
 *
...
class WCK_API_Resource {

	/** @var WCK_API_Server the API server */
	protected $server;
	...
	public function __construct( WCK_API_Server $server ) {
		...
		add_filter( 'woocommerce_klaviyo_api_endpoints', array( $this, 'register_routes' ) );
		...
		foreach ( array( 'order', 'coupon', 'customer', 'product', 'report' ) as $resource ) {
			...
			add_filter( "woocommerce_klaviyo_api_{$resource}_response", array( $this, 'maybe_add_meta' ), 15, 2 );
			add_filter( "woocommerce_klaviyo_api_{$resource}_response", array( $this, 'filter_response_fields' ), 20, 3 );
		}
	}
	...
	protected function validate_request( $id, $type, $context ) {
		...
		// validate ID
		if ( empty( $id ) )
			return new WP_Error( "woocommerce_klaviyo_api_invalid_{$resource_name}_id", sprintf( __( 'Invalid %s ID', 'woocommerce-klaviyo' ), $type ), array( 'status' => 404 ) );
		...
		if ( 'customer' !== $type ) {
			...
			// validate post type
			if ( $type !== $post_type )
				return new WP_Error( "woocommerce_klaviyo_api_invalid_{$resource_name}", sprintf( __( 'Invalid %s', 'woocommerce-klaviyo' ), $resource_name ), array( 'status' => 404 ) );
			...
			switch ( $context ) {
				...
				case 'read':
					if ( ! $this->is_readable( $post ) )
						return new WP_Error( "woocommerce_klaviyo_api_user_cannot_read_{$resource_name}", sprintf( __( 'You do not have permission to read this %s', 'woocommerce-klaviyo' ), $resource_name ), array( 'status' => 401 ) );
					break;

				case 'edit':
					if ( ! $this->is_editable( $post ) )
						return new WP_Error( "woocommerce_klaviyo_api_user_cannot_edit_{$resource_name}", sprintf( __( 'You do not have permission to edit this %s', 'woocommerce-klaviyo' ), $resource_name ), array( 'status' => 401 ) );
					break;

				case 'delete':
					if ( ! $this->is_deletable( $post ) )
						return new WP_Error( "woocommerce_klaviyo_api_user_cannot_delete_{$resource_name}", sprintf( __( 'You do not have permission to delete this %s', 'woocommerce-klaviyo' ), $resource_name ), array( 'status' => 401 ) );
					break;
			}
		}
	...
	protected function delete( $id, $type, $force = false ) {
		...
		if ( 'customer' === $type ) {
			...
			if ( $result )
				return array( 'message' => __( 'Permanently deleted customer', 'woocommerce-klaviyo' ) );
			else
				return new WP_Error( 'woocommerce_klaviyo_api_cannot_delete_customer', __( 'The customer cannot be deleted', 'woocommerce-klaviyo' ), array( 'status' => 500 ) );
			...
			if ( ! $result )
				return new WP_Error( "woocommerce_klaviyo_api_cannot_delete_{$resource_name}", sprintf( __( 'This %s cannot be deleted', 'woocommerce-klaviyo' ), $resource_name ), array( 'status' => 500 ) );

			if ( $force ) {
				return array( 'message' => sprintf( __( 'Permanently deleted %s', 'woocommerce-klaviyo' ), $resource_name ) );

			} else {

				$this->server->send_status( '202' );

				return array( 'message' => sprintf( __( 'Deleted %s', 'woocommerce-klaviyo' ), $resource_name ) );
			}
		}
	}
	 ...
	 *
	 * @since 0.9
	 * @see WCK_API_Resource::check_permission()
